...
public class KeyedMessageBuilder {
    ...
    public List<KeyedMessage> build(String logMessage) {
        ...
        Pattern timePattern = Pattern.compile("(?<cid>container_\\d+_\\d+_\\d+_\\d+) (?<timestamp>(\\d{4}|\\d{2})[-/]\\d{2}[-/]\\d{2} \\d{2}:\\d{2}:\\d{2}[,\\.]\\d{3}) (?<content>.*)");
        Matcher timeMatcher = timePattern.matcher(logMessage.toLowerCase());
        ...
        content = LogPreprocessor.preProcess(content);
